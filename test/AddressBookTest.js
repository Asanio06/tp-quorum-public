...
contract('AddressBook', function(accounts) {
  ...
  it("should detect geo boundaries properly", async () => {
    ...
    assert.equal(await addressBook.checkGeoBoundaries(milkProducer1), true, "the second participants should be in the correct geo boundaries")
    assert.equal(await addressBook.checkGeoBoundaries(milkProducer2), true, "the third participants should be in the correct geo boundaries")
